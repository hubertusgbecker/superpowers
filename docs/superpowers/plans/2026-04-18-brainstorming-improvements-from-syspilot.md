...
- [x] **AC-10:** Pressure test (Task 5.4 Step 2) shows the new flow produces a better-traced spec on the same input than the pre-change flow — recorded with before/after artifacts under [tests/ac10-pressure/](../../../tests/ac10-pressure/comparison.md). Criteria (all must hold on each pressure-test topic): (a) every spec section traces to an upstream `D-N` decision in the log; (b) trace-reviewer reports zero orphans at the brainstorm → spec handoff; (c) the new flow surfaces at least one finding the old flow missed on the same input (redundancy, gap, contradiction, or security/ops/maintainer concern). An earlier draft of AC-10 required the new spec to be "shorter or the same length"; that clause was dropped because the new flow encodes more structured information (D-N labels, decision-log header, security constraints, SLA) and longer specs are the expected outcome when they carry more information, not a regression. Two-topic evidence: both topics pass (a)/(b)/(c); new specs are 30–38% longer; new flow caught a shell-injection security constraint and a reviewer-latency SLA gap that the old flow entirely missed.
